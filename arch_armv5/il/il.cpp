...
static void ConditionExecute(LowLevelILFunction &il, uint32_t cond, ExprId trueCase)
{
    ...
    LowLevelILLabel doOp, fallthrough;
    il.AddInstruction(il.If(GetCondition(il, cond), doOp, fallthrough));
    il.MarkLabel(doOp);
    il.AddInstruction(trueCase);
...
/*
 * Conditional execution for terminator instructions (Jump, Return, TailCall).
 *
 * Terminators end a basic block, so we must structure the IL carefully:
 *   - If condition true: execute terminator (ends block)
...
static void ConditionExecuteTerminator(LowLevelILFunction &il, uint32_t cond, ExprId terminator)
{
    ...
    il.MarkLabel(doTerminator);
    il.AddInstruction(terminator);

    /* False path: fallthrough to next instruction */
    il.MarkLabel(fallthrough);
    /* Control will naturally continue to next instruction */
}

/* Conditional wrapper for multi-instruction blocks */
static void ConditionExecuteBlock(uint32_t cond, Instruction &instr, LowLevelILFunction &il,
                                  std::function<void(Instruction &instr, LowLevelILFunction &il)> conditionalCode)
{
    if (IsUnconditional(cond))
    {
        conditionalCode(instr, il);
        return;
    }
 ...
 *
 * Renamed to avoid overload ambiguity with the simpler ConditionExecute().
 */
static void ConditionExecuteBlockSized(size_t addrSize, uint32_t cond, Instruction &instr, LowLevelILFunction &il,
...
/*
 * Write to register, handling PC writes as branches.
 *
 * IMPORTANT: When reg == PC, this returns a Jump ExprId which is a TERMINATOR.
 * Callers must use ConditionExecuteTerminator() for PC writes, not ConditionExecute().
 * Failure to do so creates malformed IL with terminators mid-block.
 */
static inline ExprId SetRegisterOrBranch(LowLevelILFunction &il, Register reg, ExprId value, uint32_t flags = 0)
{
    if (reg == REG_PC)
    {
        /* PC writes are control transfers - return Jump terminator */
...
/*
 * Conditionally execute a register write, using the correct helper based on destination.
 * If destination is PC, uses ConditionExecuteTerminator since Jump is a terminator.
 * Otherwise uses regular ConditionExecute.
 */
static inline void ConditionExecuteSetRegisterOrBranch(
    LowLevelILFunction &il, uint32_t cond, Register reg, ExprId value, uint32_t flags = 0)
{
    if (reg == REG_PC)
    {
        /* PC write is a terminator (Jump) */
        ConditionExecuteTerminator(il, cond, il.Jump(value));
    }
    else
    {
        /* Normal register write */
        ConditionExecute(il, cond, il.SetRegister(get_register_size(reg), reg, value, flags));
    }
}
                        ...
                        Instruction &instr, uint64_t addr)
{
    ...
    case ARMV5_ADD:
        ...
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, instr.operands[0].reg, result, flags);
        break;

    case ARMV5_ADC:
        result = il.AddCarry(4, ReadOperand(il, instr, 1, addr), ReadOperand(il, instr, 2, addr), il.Flag(IL_FLAG_C), flags);
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, instr.operands[0].reg, result, flags);
        break;
    ...
    case ARMV5_SBC:
        result = il.SubBorrow(4, ReadOperand(il, instr, 1, addr), ReadOperand(il, instr, 2, addr), il.Not(1, il.Flag(IL_FLAG_C)), flags);
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, instr.operands[0].reg, result, flags);
        break;
    ...
    case ARMV5_CMP:
        ConditionExecute(il, instr.cond, il.Sub(4, ReadOperand(il, instr, 0, addr),
                                ReadOperand(il, instr, 1, addr), flags));
        break;

    case ARMV5_CMN:
        ConditionExecute(il, instr.cond, il.Add(4, ReadOperand(il, instr, 0, addr),
                                ReadOperand(il, instr, 1, addr), flags));
        break;

    case ARMV5_TST:
        ConditionExecute(il, instr.cond, il.And(4, ReadOperand(il, instr, 0, addr),
                                ReadOperand(il, instr, 1, addr), flags));
        break;

    case ARMV5_TEQ:
        ConditionExecute(il, instr.cond, il.Xor(4, ReadOperand(il, instr, 0, addr),
                                ReadOperand(il, instr, 1, addr), flags));
        break;
                ...
                Instruction &instr, uint64_t addr, bool thumb)
{
    ...
    case ARMV5_BL:
    {
        ConditionExecute(il, instr.cond,
                         il.Call(il.ConstPointer(addrSize, instr.operands[0].imm)));
        return true;
    }
                           ...
                           Instruction &instr, uint64_t addr, bool thumb)
{
    ...
    case ARMV5_LDMDB:
    case ARMV5_POP:
        ConditionExecuteBlockSized(addrSize, instr.cond, instr, il,
                                    ...
                                    LowLevelILFunction &il, Instruction &instr, size_t addrSize)
{
    ...
    case ARMV5_ADD:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Add(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), flags), flags);
        break;
    ...
    case ARMV5_ADC:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.AddCarry(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), il.Flag(IL_FLAG_C), flags), flags);
        break;
    ...
    case ARMV5_SUB:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Sub(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), flags), flags);
        break;
    ...
    case ARMV5_SBC:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.SubBorrow(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), il.Not(1, il.Flag(IL_FLAG_C)), flags), flags);
        break;
    ...
    case ARMV5_RSB:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Sub(get_register_size(op1.reg), ReadILOperand(il, op3, addr), ReadRegisterOrPointer(il, op2, addr), flags), flags);
        break;
    ...
    case ARMV5_RSC:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.SubBorrow(get_register_size(op1.reg), ReadILOperand(il, op3, addr), ReadRegisterOrPointer(il, op2, addr), il.Not(1, il.Flag(IL_FLAG_C)), flags), flags);
        break;
    ...
    case ARMV5_AND:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.And(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), flags), flags);
        break;
    ...
    case ARMV5_EOR:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Xor(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), flags), flags);
        break;
    ...
    case ARMV5_ORR:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Or(get_register_size(op1.reg), ReadRegisterOrPointer(il, op2, addr), ReadILOperand(il, op3, addr), flags), flags);
        break;
    ...
    case ARMV5_BIC:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.And(4, ReadRegisterOrPointer(il, op2, addr), il.Not(4, ReadILOperand(il, op3, addr)), flags), flags);
        break;
    ...
    case ARMV5_MOV:
         ...
         */
        {
            ...
            ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, value, flags);
        }
        break;
    ...
    case ARMV5_MUL:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Mult(get_register_size(op2.reg), ReadRegisterOrPointer(il, op2, addr), (op3.cls == NONE) ? ReadRegisterOrPointer(il, op1, addr) : ReadRegisterOrPointer(il, op3, addr), instr.setsFlags ? IL_FLAGWRITE_NZ : IL_FLAGWRITE_NONE));
        break;
    ...
    case ARMV5_MLA:
         ...
         */
        ConditionExecuteSetRegisterOrBranch(il, instr.cond, op1.reg, il.Add(get_register_size(op1.reg), ReadRegisterOrPointer(il, op4, addr), il.Mult(get_register_size(op2.reg), ReadRegisterOrPointer(il, op2, addr), ReadRegisterOrPointer(il, op3, addr)), instr.setsFlags ? IL_FLAGWRITE_NZ : IL_FLAGWRITE_NONE));
        break;
    ...
    case ARMV5_SMLALBB:
    {
        ...
        ConditionExecute(il, instr.cond, il.SetRegisterSplit(4, RegisterToIndex(instr.operands[1].reg), RegisterToIndex(instr.operands[0].reg), il.Add(8, acc, product)));
        break;
    }
    ...
    */
    ...
    case ARMV5_BLX:
    case ARMV5_BX:
        return LiftBranch(arch, il, instr, addr, false);
    /*
     * =======================================================================
